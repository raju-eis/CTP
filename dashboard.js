...
let __userProfile = null; // { role, email, ... } — set at boot
let __userEmail = null;   // shortcut for profile.email
let __isAdmin = false;    // true when role === "admin"
...
async function loadTicketCounters() {
  ...
  // Helper: apply user-scope filter for non-admin users
  function scopeTicket(q) {
    if (!__isAdmin && __userEmail) q = q.eq("reporter_email", __userEmail);
    return q;
  }
...
(async () => {
  await runBusy("Loading dashboard…", async () => {
    ...
    const { profile, me } = await mountNav("dashboard");

    // Store user context for role-based filtering
    __userProfile = profile;
    __userEmail = profile?.email || me?.email || null;
